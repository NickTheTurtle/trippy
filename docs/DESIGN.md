...
**Native maps use `react-native-maps`, with a web preview fallback.** Expo Go
already bundles the native map view, so the phone app can use Apple Maps on iOS
and Google Maps on Android without a custom native module. The Expo web preview
cannot render `react-native-maps`, so `DayMap.web.tsx` renders the same pins as
a list instead of crashing Metro. No Google key is needed for Expo Go. A
production Android build will need an Android Maps SDK key wired through
`app.json` and the `react-native-maps` config plugin; that is a build-time
secret, not a value to commit here.
